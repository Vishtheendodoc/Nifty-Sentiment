...
def setup_gsheet(sheet_name="NIFTY writing"):
    from datetime import datetime
    ...
    today_tab = datetime.today().strftime("Sheet_%Y%m%d")
    try:
        sheet = spreadsheet.worksheet(today_tab)
    except gspread.exceptions.WorksheetNotFound:
        sheet = spreadsheet.add_worksheet(title=today_tab, rows="1000", cols="10")
        sheet.append_row(["TimeSlot", "Flow_Type", "Weighted_Activity", "MFI", "UnderlyingValue"])
        ...
        try:
            ...
            st.warning(f"⚠️ Formatting skipped: {e}")

    return sheet


# def apply_conditional_coloring(sheet):
    from gspread_formatting import format_cell_range, Color, CellFormat

    records = sheet.get_all_records()
    for i, row in enumerate(records, start=2):
        flow = row.get("Flow_Type", "")
        weight = float(row.get("Weighted_Activity", 0))
        scale = min(weight / 10_000_000, 1.0)

        if "Put Write" in flow:
            # Light green base + slight intensity
            color = Color(0.85 - 0.4 * scale, 1.0, 0.85 - 0.4 * scale)
        elif "Call Short" in flow:
            # Light red base + slight intensity
            color = Color(1.0, 0.85 - 0.4 * scale, 0.85 - 0.4 * scale)
        else:
            continue

        fmt = CellFormat(backgroundColor=color)
        format_cell_range(sheet, f"A{i}:E{i}", fmt)


def get_support_resistance_levels(sheet, threshold=5000000):
    import pandas as pd
    records = sheet.get_all_records()
    df = pd.DataFrame(records)

    # Filter significant writing only
    put_df = df[df["Flow_Type"].str.contains("Put Write", na=False) &
                (df["Weighted_Activity"] >= threshold)]
    
    call_df = df[df["Flow_Type"].str.contains("Call Short", na=False) &
                 (df["Weighted_Activity"] >= threshold)]

    # Weighted average of underlying values based on writing volume
    support = None
    resistance = None

    if not put_df.empty:
        support = round(
            (put_df["UnderlyingValue"] * put_df["Weighted_Activity"]).sum() /
            put_df["Weighted_Activity"].sum(), 2
        )

    if not call_df.empty:
        resistance = round(
            (call_df["UnderlyingValue"] * call_df["Weighted_Activity"]).sum() /
            call_df["Weighted_Activity"].sum(), 2
        )
